<?php
	...
	// DEBUG
	error_log("listaConversas.php - Variáveis recebidas: ID=" . $idAtendimento . " | Numero=" . $numero . " | Canal=" . $idCanal);
	if (mysqli_connect_error()) {
		error_log("ERRO DB: " . mysqli_connect_error());
	...
	if( $idAtendimento === "att" ){
		$strSQL = "SELECT tma.chatid, tma.id, tma.seq, tma.numero, tma.msg, tma.resp_msg, tma.dt_msg, tma.hr_msg, tma.id_atend, 
		       ta.id as anexo_id, ta.numero as anexo_numero, ta.seq as anexo_seq, ta.tipo_arquivo, ta.nome_original, ta.base64, 
		       tma.situacao, tma.reagir, tma.reacao, tma.reacaorec
		...
		if( $idAtendimento === "all" ){
			$strSQL = "SELECT tma.chatid, tma.id, tma.seq, tma.numero, tma.msg, tma.resp_msg, tma.dt_msg, tma.hr_msg, tma.id_atend, 
			       ta.id as anexo_id, ta.numero as anexo_numero, ta.seq as anexo_seq, ta.tipo_arquivo, ta.nome_original, ta.base64, 
			       tma.situacao, tma.reagir, tma.reacao, tma.reacaorec, tma.apagada, tma.apagada
							FROM tbmsgatendimento tma
								LEFT JOIN tbanexos ta ON tma.id_anexo = ta.id
			...
			$strSQL = "SELECT tma.chatid, tma.id, tma.seq, tma.numero, tma.msg,  tma.resp_msg, tma.dt_msg, tma.hr_msg, tma.id_atend, 
			       ta.id as anexo_id, ta.numero as anexo_numero, ta.seq as anexo_seq, ta.tipo_arquivo, ta.nome_original, ta.base64, 
			       tma.situacao, tma.reagir, tma.reacao, tma.reacaorec, tma.apagada
							FROM tbmsgatendimento tma
								LEFT JOIN tbanexos ta ON tma.id_anexo = ta.id
								WHERE tma.numero = '".$numero."' AND  tma.id = '".$idAtendimento."'
									ORDER BY seq";
	...
	while( $objConversa = mysqli_fetch_object($qryConversa) ){
		...
		// $S_TIPO:= 'Atendimento';  Estilo da Exibição da Mensagem do Usuario do Chat //
			...
			if( strpos($mensagem, 'BEGIN:VCARD') !== false ){
				...
				echo '	<div class="_3zb-j ZhF0n">
								<span dir="ltr" class="selectable-text invisible-space message-text">'. $mensagem .'</span>
							</div>							
							'.$reascteret.'							
							<div class="_2f-RV">
								<div class="_1DZAH" role="button">
